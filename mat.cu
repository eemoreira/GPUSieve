...
void runTest(void) {
  ...
  for (int i = 0; i < doublings; i++) {
    ...
    serialElapsedTime = (serialEnd.tv_sec - serialStart.tv_sec) * 1000.0;
    serialElapsedTime += (serialEnd.tv_usec - serialStart.tv_usec) / 1000.0;

    parallelElapsedTime = (parallelEnd.tv_sec - parallelStart.tv_sec) * 1000.0;
    parallelElapsedTime +=
    ...
    printf("%d x %d: Serial: %f\t\tParallel %f\t(%f%% Speedup)\n", width, width,
           serialElapsedTime, parallelElapsedTime, speedup);
